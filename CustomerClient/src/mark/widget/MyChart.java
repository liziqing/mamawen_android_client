...
public class MyChart extends View {
	 ...
	 * 内边距
	 */
	private float padding_top = 40, padding_bottom = 40, padding_left = 40,
			padding_right = 40;
	/**
	 * 图表的上下左右坐标
	 */
	 ...
	 * 控件刻度标签的字体颜色
	 */
	private int label_text_colorX = Color.BLACK,
	...
	private void init() {
		...
		// 初始化控件背景色
		backColor = Color.parseColor("#f5f5f5");
		// 初始化标签字体颜色
		...
		// 初始化图表背景色
		chart_background_color = Color.WHITE;
		// 初始化
	...
	private void drawGrid(Canvas canvas) {
		...
		for (int i = 0; i < y_labels.length; i++) {
			canvas.drawLine(chartLeft, padding_top + VSP * i, chartRight,
					padding_top + VSP * i, paint);
		}
	}
	...
	private void drawLabelY(Canvas canvas) {
		...
		if (x_labels != null) {
			VSP = (chartBottom - chartTop) / (y_labels.length - 1);
			for (int i = 0; i < y_labels.length; i++) {
				canvas.drawText(y_labels[i] + "", padding_left / 2, chartBottom
						- i * VSP, paint);
			}
		}
	}
